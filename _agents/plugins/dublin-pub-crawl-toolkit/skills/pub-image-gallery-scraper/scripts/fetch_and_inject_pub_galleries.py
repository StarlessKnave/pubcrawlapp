...
def inject_into_index_html():
    ...
    gallery_popup_snippet = """        <!-- Pub Pin Menu Photo Gallery Carousel -->
        <div class="mb-3 rounded-lg overflow-hidden border border-[#d9ac5e]/30 bg-[#12070b]">
          <div class="relative h-36 sm:h-40 w-full bg-[#0a0406] group">
            ...
            <a href="${pub.gbpUrl || '#'}" target="_blank" rel="noopener"
               class="absolute top-2 right-2 px-2 py-1 rounded bg-[#0a0406]/85 hover:bg-[#d9ac5e] text-[#d9ac5e] hover:text-[#0a0406] border border-[#d9ac5e]/50 text-[10px] font-headline font-bold uppercase tracking-wider transition-all flex items-center gap-1 shadow">
            ...
            <div class="absolute bottom-2 left-2 right-2 flex items-center justify-between gap-2">
              ...
              <div class="flex items-center gap-1 shrink-0">
                <button onclick="switchPinGalleryPhoto('${pub.id}', 0)" id="pin-tab-${pub.id}-0" class="w-5 h-5 rounded-full bg-[#d9ac5e] text-[#0a0406] font-bold text-[10px] flex items-center justify-center">1</button>
                <button onclick="switchPinGalleryPhoto('${pub.id}', 1)" id="pin-tab-${pub.id}-1" class="w-5 h-5 rounded-full bg-black/70 text-[#ede5d8] border border-white/20 font-bold text-[10px] flex items-center justify-center">2</button>
                <button onclick="switchPinGalleryPhoto('${pub.id}', 2)" id="pin-tab-${pub.id}-2" class="w-5 h-5 rounded-full bg-black/70 text-[#ede5d8] border border-white/20 font-bold text-[10px] flex items-center justify-center">3</button>
              </div>
            </div>
          </div>
        </div>
...
"""
  ...
  <div id="pub-gallery-lightbox-modal" class="fixed inset-0 z-[9999] hidden bg-black/90 backdrop-blur-md flex items-start sm:items-center justify-center p-4 overflow-y-auto modal-mobile-safe">
    <div class="relative max-w-3xl w-full guinness-panel rounded-xl p-4 sm:p-6 border border-[#d9ac5e]/50 my-auto">
      <div class="flex items-center justify-between mb-3">
        <span id="lightbox-main-caption" class="font-headline font-bold text-sm sm:text-base text-[#d9ac5e] uppercase tracking-wider">Pub Gallery</span>
        <button onclick="closePubGalleryLightbox()" class="min-w-[44px] min-h-[44px] rounded-lg bg-[#180d12] text-[#ede5d8] hover:text-[#d9ac5e] border border-[#2a1720] flex items-center justify-center font-bold text-lg">✕</button>
      </div>
      <div class="rounded-lg overflow-hidden bg-[#0a0406] border border-[#2a1720] max-h-[65vh] flex items-center justify-center">
        <img id="lightbox-main-img" src="" alt="Pub Fullscreen Photo" class="w-full h-auto max-h-[65vh] object-contain" />
      </div>
      <div class="mt-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <button onclick="closePubGalleryLightbox()" class="br-button-secondary px-4 py-2.5 text-xs">Close Gallery</button>
      </div>
    </div>
  </div>
"""
    if 'id="pub-gallery-lightbox-modal"' not in content:
